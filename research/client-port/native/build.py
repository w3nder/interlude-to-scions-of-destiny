...
common=['-std=c++17','-O2','-Wall','-Wextra','-Werror','-fno-exceptions','-fno-rtti',str(ROOT/'protocol_core.cpp'),str(ROOT/'clan_codec.cpp'),str(ROOT/'clan_self.cpp'),str(ROOT/'schema_codec.cpp'),str(ROOT/'structured_codec.cpp'),str(ROOT/'outbound_policy.cpp'),str(ROOT/'asset_codec.cpp'),str(ROOT/'character_codec.cpp'),str(ROOT/'pledge_bridge.cpp'),str(ROOT/'status_codec.cpp')]
subprocess.run(['clang++',*common,'-dynamiclib','-o',str(BUILD/'libl2k_protocol.dylib')],check=True)
subprocess.run(['i686-w64-mingw32-g++',*common,str(ROOT/'bootstrap.cpp'),str(ROOT/'login_hooks.cpp'),str(ROOT/'game_trace.cpp'),str(ROOT/'local_ui.cpp'),str(ROOT/'status_bridge.cpp'),'-shared','-static-libgcc','-static-libstdc++',
